...
const CalcArea = () => {
  ...
  return (
    <div className='container'>
      ...
      <p>
        Gib eine beliebige Fläche ein, um zu sehen, dass Dido schlau gehandelt
        hat, als sie die Kuhhaut in einem Kreis ausgelegt hat. <br></br>Drücke
        dann ENTER.
      </p>
      ...
      {alert && (
        <div style={{ color: 'red', margin: '1rem', fontSize: 'larger' }}>
          Bitte eine Zahl eingeben
        </div>
      )}
      <div>
        <p>
          Kreis: {circle > 0 ? circle : ''} {input > 0 ? 'cm' : ''}
        </p>
        <p>
          Rechteck: {square > 0 ? square : ''} {input > 0 ? 'cm' : ''}
        </p>
        <p>
          Dreieck: {triangle > 0 ? triangle : ''} {input > 0 ? 'cm' : ''}
        </p>
      </div>
    </div>
  );
